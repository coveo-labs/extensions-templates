...
def normalize_document_based_on_meta(targeted_metadata_name, targeted_metadata_values, new_metadata_name, new_metadata_value, new_metadata_field):
    ...
    try:
        ...
        not_affected_metadata_values = ";".join(
            list(set(metadata_values).difference(values)))

        if not not_affected_metadata_values:
            new_value = (new_metadata_value or metadata[new_metadata_field])
            log('Setting values on {} with : {}'.format(
                new_metadata_name, new_value.encode('utf8')))
...
if 'targeted_metadata_values' not in parameters:
    ...
    raise Exception(
        'Supply a targeted_metadata_values in the parameters of ext')
...
if 'new_metadata_value' not in parameters and 'new_metadata_field' not in parameters:
    ...
    raise Exception(
        'Supply a new_metadata_value or new_metadata_field in the parameters of ext')
...
normalize_document_based_on_meta(parameters['targeted_metadata_name'], parameters['targeted_metadata_values'],
                                 parameters['new_metadata_name'], new_metadata_value, new_metadata_field)
